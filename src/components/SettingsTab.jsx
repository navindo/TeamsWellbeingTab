...
export default function SettingsTab() {
  ...
  // Small helpers for response logging
  ...
  useEffect(() => {
    // 1) Load API base from runtime /config.json, else fallback to baked env
    ...
    fetch("/config.json", { cache: "no-store" })
      ...
      .then((finalBase) => {
        ...
        // 2) Teams init + SSO token
        microsoftTeams.app.initialize().then(() => {
          addLog("[Teams] SDK initialized. Requesting auth token...");

          // Safety timeout if token never comes back
          ...
          microsoftTeams.authentication.getAuthToken({
            successCallback: (token) => {
              ...
              setObjectId(oid);
              setAuthToken(token);
              addLog(`[Teams] SSO token received. ObjectId=${oid}`);
              ...
              fetch(url, {
                method: "GET"
                // If your API expects the Teams SSO token, add:
                ...
                .catch((err) => addLog(`[Error] Failed to load settings: ${err?.message || err}`))
  ...
  // Always show logs (even while loading)
  return (
